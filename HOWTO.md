How to run your own Electrum monacoCoin server
========================================
...
Electrum monacoCoin server on Linux. It is structured as a series of steps you need to
...
The most up-to date version of this document is available at:
    ...
    https://github.com/monacocoin-net/electrum-monacoCoin-server
...
document, we assume that user is called 'monacoCoin'). We also assume the
monacoCoin user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run monacoCoind and keep a copy of the full blockchain,
which is roughly 55 GB (February 2016). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit monacoCoind 
...
only leaving spare cycles for monacoCoind. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running monacoCoind and Electrum monacoCoin server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `monacoCoind` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser monacocoin --disabled-password
    $ sudo apt-get install git
    $ sudo su - monacocoin
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/monacoCoin/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download monacoCoind

Recommend downloading latest version directly from monacoCoin.org

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libevent-dev unzip
    $ sudo su - monacocoin
    $ cd ~/src && wget https://github.com/monacocoin-net/monacoCoin-Core/releases/download/0.12.1.5/monacoCoinCore-0.12.1.5-linux64-cli.zip
    $ unzip monacoCoinCore-0.12.1.5-linux64-cli.zip
	$ chmod +x monacoCoind monacoCoin-cli monacoCoin-tx
    $ cd monacoCoin-0.12.0/bin
    $ cp -a monacoCoind monacoCoin-cli monacoCoin-tx ~/bin

### Step 3. Configure and start monacoCoind

In order to allow Electrum monacoCoin to "talk" to `monacoCoind`, we need to set up an RPC
username and password for `monacoCoind`. We will then start `monacoCoind` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.monacoCoin
    $ $EDITOR ~/.monacoCoin/monacoCoin.conf

Write this in `monacoCoin.conf`:
...
If you have an existing installation of monacoCoind and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ monacoCoind -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `monacoCoind`:

    $ monacoCoind

Allow some time to pass for `monacoCoind` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ monacoCoin-cli getblockchaininfo

Before starting the Electrum monacoCoin server your monacoCoind should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start monacoCoind at boot
time, running as the 'monacoCoin' user. Check your system documentation to
find out the best way to do this.

### Step 4. Download and install Electrum monacoCoin server

We will download the latest git snapshot for Electrum to configure and install it:

    $ cd ~
    $ git clone https://github.com/monacocoin-net/electrum-monacoCoin-server.git
    $ cd electrum-monacoCoin-server
...
### Optional Step 5: Install Electrum monacoCoin dependencies manually

Electrum monacoCoin server depends on various standard Python libraries and leveldb. These will usually be
...
Electrum monacoCoin server uses leveldb to store transactions. You can choose
...
and further instructions from the Electrum monacoCoin full archival server foundry at a later time.
...
server.key corresponds to `ssl_keyfile=` in your Electrum monacoCoin server config.
...
### Step 9. Configure Electrum monacoCoin server

Electrum monacoCoin reads a config file (/etc/electrum-monacoCoin.conf) when starting up. This
file includes the database setup, monacoCoind RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-monacoCoin.conf
...
Electrum monacoCoin server currently needs quite a few file handles to use leveldb. It also requires
file handles for each connection made to the server. It's good practice to increase the
open files limit to 64k.

The "configure" script will take care of this and ask you to create a user for running electrum-monacoCoin-server.
If you're using the user `monacoCoin` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "monacocoin hard nofile 65536" >> /etc/security/limits.conf
     echo "monacocoin soft nofile 65536" >> /etc/security/limits.conf
...
Check if the limits are changed either by logging with the user configured to run Electrum monacoCoin server as. Example:
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u monacocoin -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close monacoCoind for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting monacoCoind (together with electrum-server) on a weekly basis to clear out unconfirmed
   transactions from the local the memory pool which did not propagate over the network.

### Step 11. (Finally!) Run Electrum monacoCoin server

The magic moment has come: you can now start your Electrum monacoCoin server as root (it will su to your unprivileged user):
...
You should see this in the log file:
    ...
    starting Electrum monacoCoin server
...
If you want to stop Electrum monacoCoin server, use the 'stop' command:
...
### Step 12. Test the Electrum monacoCoin server
...
current server and connect to your new Electrum monacoCoin server. You should see your
addresses and transactions history. You can see the number of blocks and
response time in the server selection window. You should send/receive some
monacoCoins to confirm that everything is working properly.
...
If you're operating a public Electrum monacoCoin server please subscribe
