IRC is used by Electrum monacoCoin server to find 'peers' - other Electrum monacoCoin servers. The
current list can be seen by running:

  electrum-monacoCoin-server peers

The following config file options are used by the IRC part of Electrum monacoCoin server:
...
the Electrum monacoCoin server will run in private mode (default). In private
mode, `electrum-monacoCoin-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Electrum monacoCoin
...
your Electrum monacoCoin server instead of `host`. It is used as part of the name
...
composing the IRC nickname to identify your server on #electrum-monacoCoin .
